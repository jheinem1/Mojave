...
function transformArrayLiteralExpression(state, node) {
    if (!node.elements.find(element => byots_1.default.isSpreadElement(element))) {
        return LuauAST_1.default.array(ensureTransformOrder_1.ensureTransformOrder(state, node.elements));
    }
    const ptr = pointer_1.createArrayPointer();
    ...
    for (let i = 0; i < node.elements.length; i++) {
        ...
        if (byots_1.default.isSpreadElement(element)) {
            if (LuauAST_1.default.isArray(ptr.value)) {
                pointer_1.disableArrayInline(state, ptr);
                updateLengthId();
            }
            assert_1.assert(LuauAST_1.default.isAnyIdentifier(ptr.value));
            const type = state.getType(element.expression);
            const addIterableToArrayBuilder = getAddIterableToArrayBuilder_1.getAddIterableToArrayBuilder(state, element.expression, type);
            const spreadExp = transformExpression_1.transformExpression(state, element.expression);
            ...
            const [expression, prereqs] = state.capture(() => transformExpression_1.transformExpression(state, element));
